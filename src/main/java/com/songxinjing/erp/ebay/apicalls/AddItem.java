...
import com.songxinjing.erp.ebay.CategoryHelper;
...
public class AddItem extends SDKTestCase {
	...
	public void testAddItem2() throws Exception {
		...
		item.getPrimaryCategory().setCategoryID(Integer.toString(CATEGORY_CUSTOM_ENABLED));

		// add custom item specifics if the category supports
		boolean isCustomEnabled = CategoryHelper.getCategoryFeature(CATEGORY_CUSTOM_ENABLED, apiContext,
		...
		returnPolicyType.setRefundOption(RefundOptionsCodeType.MONEY_BACK_OR_EXCHANGE.value());
		returnPolicyType.setReturnsWithinOption(ReturnsWithinOptionsCodeType.DAYS_14.value());
		returnPolicyType.setShippingCostPaidByOption(ShippingCostPaidByOptionsCodeType.BUYER.value());
		returnPolicyType.setReturnsAcceptedOption(ReturnsAcceptedOptionsCodeType.RETURNS_ACCEPTED.value());
