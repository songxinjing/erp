...
public class GetCategoryFeatures extends SDKTestCase {
	 ...
	 * GetCategoryFeaturesCall api = new GetCategoryFeaturesCall(apiContext);
	 * api.setSite(SiteCodeType.US); DetailLevelCodeType[] detailLevels = new
	 * DetailLevelCodeType[]{ DetailLevelCodeType.RETURN_ALL };
	 * api.setDetailLevel(detailLevels);
	 * 
	 * FeatureIDCodeType[] features = new FeatureIDCodeType[]{
	 * FeatureIDCodeType.ITEM_SPECIFICS_ENABLED }; api.setFeatureIDs(features);
	 * api.getCategoryFeatures();
	 * 
	 * CategoryFeatureType[] res = api.getReturnedCategory(); for(int i = 0 ;i <
	 * res.length; i++){ if(res[i].getItemSpecificsEnabled().compareTo(
	 * ItemSpecificsEnabledCodeType.ENABLED)==0){
